...
const StaffQuizzes = () => {
  ...
  const [form, setForm] = useState({ title: '', description: '', subjectId: '', timeLimit: '', dueDate: '', questions: [{ ...emptyQuestion }] })
  ...
  const updateQuestion = (i, field, value) => setForm(p => { const qs = [...p.questions]; qs[i] = { ...qs[i], [field]: value }; return { ...p, questions: qs } })
  const updateOption   = (qi, oi, value) => setForm(p => { const qs = [...p.questions]; const opts = [...qs[qi].options]; opts[oi] = value; qs[qi] = { ...qs[qi], options: opts }; return { ...p, questions: qs } })
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title="📝 Create Quiz" size="lg">
        <form onSubmit={handleCreate} className="space-y-5 max-h-[70vh] overflow-y-auto pr-1">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div><label className="label">Time Limit (min)</label><input type="number" className="input-field" value={form.timeLimit} onChange={e => setForm(p => ({ ...p, timeLimit: e.target.value }))} min="1" /></div>
            <div className="col-span-2"><label className="label">Due Date</label><input type="datetime-local" className="input-field" value={form.dueDate} onChange={e => setForm(p => ({ ...p, dueDate: e.target.value }))} /></div>
          </div>
          ...
          <div className="space-y-4">
            ...
            {form.questions.map((q, qi) => (
              <div key={qi} className="border-2 border-orange-100 rounded-2xl p-4 space-y-3 bg-amber-50/40">
                ...
                <input className="input-field" placeholder="Question text..." value={q.text} onChange={e => updateQuestion(qi, 'text', e.target.value)} required />
