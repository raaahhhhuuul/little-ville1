...
const AdminSalary = () => {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const res = await getSalaries({ year: yearFilter })
  ...
  const columns = [
    ...
    { key: 'deductions', header: 'Deductions', render: r => <span className="text-red-600">-${r.deductions.toFixed(2)}</span> },
    { key: 'bonus', header: 'Bonus', render: r => <span className="text-green-600">+${r.bonus.toFixed(2)}</span> },
    { key: 'netSalary', header: 'Net Salary', render: r => <span className="font-semibold text-gray-900">${r.netSalary.toFixed(2)}</span> },
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <input type="number" step="0.01" className="input-field" value={form.baseSalary} onChange={e => setForm(p => ({ ...p, baseSalary: e.target.value }))} required min="0" />
            </div>
            <div>
              <label className="label">Deductions</label>
              <input type="number" step="0.01" className="input-field" value={form.deductions} onChange={e => setForm(p => ({ ...p, deductions: e.target.value }))} min="0" />
            </div>
            <div>
              <label className="label">Bonus</label>
              <input type="number" step="0.01" className="input-field" value={form.bonus} onChange={e => setForm(p => ({ ...p, bonus: e.target.value }))} min="0" />
            </div>
          </div>
          <div className="bg-gray-50 rounded-lg p-3 text-sm">
            <span className="text-gray-600">Net Salary: </span>
            <span className="font-bold text-gray-900">
              ${((Number(form.baseSalary) || 0) - (Number(form.deductions) || 0) + (Number(form.bonus) || 0)).toFixed(2)}
            </span>
          </div>
          <div>
            <label className="label">Reason / Notes</label>
            <textarea className="input-field" rows={2} value={form.reason} onChange={e => setForm(p => ({ ...p, reason: e.target.value }))} />
          </div>
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={() => setModalOpen(false)} className="btn-secondary flex-1">Cancel</button>
            <button type="submit" disabled={submitting} className="btn-primary flex-1 flex items-center justify-center gap-2">
              {submitting ? <Loader2 size={14} className="animate-spin" /> : <DollarSign size={14} />} Save
            </button>
          </div>
        </form>
      </Modal>
