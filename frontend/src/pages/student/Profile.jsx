      ...
      <div className="max-w-2xl space-y-6">
        ...
        <div className="card">
          <div className="flex items-center gap-5">
            <div className="w-20 h-20 bg-gradient-to-br from-sky-400 to-blue-600 rounded-3xl flex items-center justify-center shrink-0 shadow-lg shadow-sky-200">
              <span className="text-white text-2xl font-black">{initials || '🎒'}</span>
            </div>
            <div>
              <h2 className="text-xl font-black text-gray-800">{form.firstName} {form.lastName}</h2>
              <p className="text-gray-500 text-sm font-medium">{user?.email}</p>
              <span className="badge-blue mt-2 inline-flex">🎒 Student</span>
            </div>
          </div>
        </div>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="flex gap-3">
                ...
                <button type="submit" disabled={submitting} className="btn-primary flex-1 flex items-center justify-center gap-2">
                  {submitting ? <><Loader2 size={14} className="animate-spin" /> Saving...</> : <><Save size={14} /> Save</>}
                </button>
              </div>
            </form>
          ) : (
